...
import { useProfileStore, useHostProfile } from '@/lib/store/useProfileStore'
...
export function ProfileList({
  ...
  // Store - Only show guest profiles (not host)
  const profiles = useProfileStore((s) => s.guestProfiles)
  const hostProfile = useHostProfile()
  const removeGuestProfile = useProfileStore((s) => s.removeGuestProfile)
  const setHostProfile = useProfileStore((s) => s.setHostProfile)

  // State
  const [editingProfile, setEditingProfile] =
    useState<LocalPlayerProfile | null>(null)
  ...
  const handleDeleteConfirm = useCallback(
    async (profileId: string) => {
      ...
      try {
        ...
        // Only delete guest profiles (not host)
        if (profileToRemove && !profileToRemove.isHost) {
          removeGuestProfile(profileId)
        }
    ...
    <div className={`mx-auto w-full max-w-md ${className}`}>
      ...
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-foreground text-xl font-bold">Joueurs Invités</h2>
      ...
      {profiles.length === 0 ? (
        ...
        >
          <div className="bg-muted mb-4 inline-flex size-16 items-center justify-center rounded-full">
              ...
              {profiles.map((profile) => {
                ...
                return (
                  ...
                  >
                    {/* Avatar */}
                    <Avatar
                      className={`size-12 ${isHost ? 'ring-primary ring-offset-background ring-2 ring-offset-2' : ''}`}
                    >
                      {profile.avatarUri ? (
                        <AvatarImage
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        {isHost && (
                          <Badge className="bg-primary/20 text-primary hover:bg-primary/30 gap-1 border-0">
