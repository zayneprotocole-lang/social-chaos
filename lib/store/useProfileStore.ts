    ...
    persist(
        (set, get) => ({
            ...
            updateProfile: (input: UpdateProfileInput): LocalPlayerProfile | null => {
                ...
                const profileIndex = profiles.findIndex((p) => p.id === validatedInput.id)
                if (profileIndex === -1) {
                ...
                const updatedProfile: LocalPlayerProfile = {
                    ...
                    preferences: {
                        want: validatedInput.preferences?.want ?? existingProfile.preferences?.want ?? [],
                        avoid: validatedInput.preferences?.avoid ?? existingProfile.preferences?.avoid ?? [],
                    },
                }
            ...
            deleteProfile: (id: string): boolean => {
                ...
                set((state) => ({
                    ...
                    hostProfileId: state.hostProfileId === id ? null : state.hostProfileId,
                }))
...
export function useHasHostProfile() {
    return useProfileStore((state) => state.hostProfileId !== null || state.profiles.some((p) => p.isHost))
}
