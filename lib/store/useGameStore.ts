...
export const useGameStore = create<GameState>((set) => ({
    ...
    useReroll: (playerId: string, nextDare: Dare) => set((state) => {
        if (!state.session) return {}
        const updatedPlayers = state.session.players.map(p => {
            if (p.id !== playerId) return p
    ...
    useSwap: (initiatorId: string, targetPlayerId: string) => set((state) => {
        if (!state.session) return {}
    ...
    nextTurn: (nextPlayerId, nextDare) => set((state) => {
        if (!state.session) return {}

        let newSession = { ...state.session }

        newSession.playersPlayedThisRound = (newSession.playersPlayedThisRound || 0) + 1

        const activePlayersCount = newSession.players.filter(p => !p.isPaused).length

        if (newSession.playersPlayedThisRound >= activePlayersCount) {
            newSession.roundsCompleted += 1
            newSession.playersPlayedThisRound = 0

            if (newSession.isProgressiveMode && newSession.roundsCompleted > 0 && newSession.roundsCompleted % 2 === 0) {
                if (newSession.settings.difficulty < 4) {
                    const nextDifficulty = (newSession.settings.difficulty + 1) as DifficultyLevel
                    newSession.settings.difficulty = nextDifficulty
                    // V8.0 timer durations
                    if (DIFFICULTY_CONFIG[nextDifficulty]) {
                        newSession.settings.timerDuration = DIFFICULTY_CONFIG[nextDifficulty].timer
                    }
                }
            }
        }
    ...
    togglePlayerPause: (playerId) => set((state) => {
        if (!state.session) return {}
        const updatedPlayers = state.session.players.map(p =>
