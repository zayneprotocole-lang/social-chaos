...
interface MentorEleveStore {
    ...
    // Lecture
    getActiveLink: (profileId1: string, profileId2: string) => MentorEleveLink | null
    getConsumedLink: (profileId1: string, profileId2: string) => MentorEleveLink | null
    ...
    updateLink: (linkId: string, mentorProfileId: string, eleveProfileId: string) => void  // Mise à jour des rôles
    ...
    persist(
        (set, get) => ({
            ...
            getActiveLink: (profileId1, profileId2) => {
                return get().links.find(link =>
                    !link.isConsumed &&
                    ((link.mentorProfileId === profileId1 && link.eleveProfileId === profileId2) ||
                        (link.mentorProfileId === profileId2 && link.eleveProfileId === profileId1))
                ) || null
            },
            ...
            getLinksForProfile: (profileId) => {
                return get().links.filter(link =>
                    link.mentorProfileId === profileId || link.eleveProfileId === profileId
                )
            },
 ...
 */
export function useHasAccompagnement(profileId1: string | undefined, profileId2: string | undefined): boolean {
 ...
 */
export function detectActiveDuos(players: { id: string, profileId?: string, name: string }[]): ActiveDuo[] {
    ...
    for (let i = 0; i < playersWithProfile.length; i++) {
        for (let j = i + 1; j < playersWithProfile.length; j++) {
            ...
            const link = allLinks.find(l =>
                !l.isConsumed &&
                ((l.mentorProfileId === player1.profileId && l.eleveProfileId === player2.profileId) ||
                    (l.mentorProfileId === player2.profileId && l.eleveProfileId === player1.profileId))
            )

            if (link) {
                activeDuos.push({
                    linkId: link.id,
                    mentorPlayerId: link.mentorProfileId === player1.profileId ? player1.id : player2.id,
                    elevePlayerId: link.eleveProfileId === player1.profileId ? player1.id : player2.id,
 ...
 */
export function enrichPlayersWithAccompagnement<T extends { id: string, profileId?: string, name: string }>(
    players: T[],
    activeDuos: ActiveDuo[]
): T[] {
